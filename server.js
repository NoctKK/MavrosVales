...
io.on('connection', (socket) => {
    ...
    socket.on('playCard', (data) => {
        ...
        if (isValid) {
            ...
            if (card.value === 'A') {
                if (!activeSuit && card.suit === topCard.suit) activeSuit = null; // Σαν φύλλο
    ...
    socket.on('drawCard', () => {
        ...
        if (penaltyStack > 0) {
            ...
            advanceTurn(1); broadcastUpdate(); return;
        }
...
function processCardLogic(card, currentPlayer) {
    ...
    else if (card.value === '9') { steps = (playerOrder.length === 2) ? 0 : 2; advance = (playerOrder.length !== 2); }
