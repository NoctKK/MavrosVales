...
let gameStarted = false, roundHistory = [], roundStarterIndex = 0;

// AUTO-RESET LOGIC (ΜΟΝΟ ΠΡΙΝ ΤΟ ΠΑΙΧΝΙΔΙ)
let lobbyTimer = null;
function resetLobbyTimer() {
    if (gameStarted) { if(lobbyTimer) clearTimeout(lobbyTimer); return; }
    if (lobbyTimer) clearTimeout(lobbyTimer);
    lobbyTimer = setTimeout(() => {
        if (!gameStarted && Object.keys(players).length > 0) {
            let activeFound = Object.values(players).some(p => p.connected);
            if (!activeFound) {
                players = {}; playerOrder = []; roundHistory = [];
...
io.on('connection', (socket) => {
    ...
    socket.on('playCard', (data) => {
        ...
        // "Σαν φύλλο" check: Αν ο Άσσος πέσει σε ίδιο σχήμα, δεν ρωτάει
        let isAsFyllo = (card.value === 'A' && !activeSuit && card.suit === topCard.suit);
        ...
        if (isValid) {
            ...
            if (p.hand.length === 0) {
                ...
                let isPenaltyEnd = false;

                if (card.value === 'J' && card.color === 'black') {
                    penaltyStack += 10;
                    let totalToDraw = penaltyStack;
                    for(let i=0; i<totalToDraw; i++) { if(deck.length === 0) refillDeck(); players[victimId].hand.push(deck.pop()); }
                    io.emit('notification', `Κλείσιμο με Μαύρο Βαλέ! +${totalToDraw} στον/στην ${players[victimId].name}!`);
                    isPenaltyEnd = true;
                } else if (card.value === '7') {
                    penaltyStack += 2;
                    let totalToDraw = penaltyStack;
                    for(let i=0; i<totalToDraw; i++) { if(deck.length === 0) refillDeck(); players[victimId].hand.push(deck.pop()); }
                    io.emit('notification', `Κλείσιμο με 7! +${totalToDraw} στον/στην ${players[victimId].name}!`);
                    isPenaltyEnd = true;
                } else if (card.value === '2') {
                    // Κλείσιμο με 2: Ο ΠΡΟΗΓΟΥΜΕΝΟΣ τρώει 1
                    let prevId = playerOrder[(turnIndex - direction + playerOrder.length) % playerOrder.length];
                    if(deck.length === 0) refillDeck(); players[prevId].hand.push(deck.pop());
                    io.emit('notification', `Κλείσιμο με 2! +1 στον/στην ${players[prevId].name}!`);
                }

                broadcastUpdate(); 
                setTimeout(() => { handleRoundEnd(socket.id, card.value === 'A'); }, isPenaltyEnd ? 3500 : 1000);
                return;
            }

            if (isAsFyllo) {
                activeSuit = null; io.emit('notification', 'Σαν φύλλο! 🃏');
    ...
    socket.on('drawCard', () => {
        ...
        for(let i=0; i<count; i++) { if(deck.length === 0) refillDeck(); if(deck.length > 0) p.hand.push(deck.pop()); }
    ...
    socket.on('passTurn', () => {
        ...
        if (!players[socket.id].hasDrawn) return socket.emit('notification', 'Τράβα πρώτα!');
        advanceTurn(1); broadcastUpdate();
    });
    socket.on('chatMessage', (msg) => { if (players[socket.id]) io.emit('chatUpdate', { name: players[socket.id].name, text: msg }); });
});

function processCardLogic(card, currentPlayer) {
    let advance = true, steps = 1, isStart = (!currentPlayer || !currentPlayer.id);
    if (card.value === '2' && !isStart) {
        let victimId = playerOrder[(turnIndex - direction + playerOrder.length) % playerOrder.length];
    ...
    else if (card.value === '9') { steps = 2; if (playerOrder.length === 2) advance = false; }
...
function refillDeck() {
    if (discardPile.length > 1) {
        let top = discardPile.pop(); deck = [...discardPile].sort(() => Math.random() - 0.5);
        discardPile = [top]; io.emit('notification', '🔄 Ανακάτεμα!');
    }
}
