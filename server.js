...
io.on('connection', (socket) => {
    ...
    socket.on('playCard', (data) => {
        ...
        if (penaltyStack > 0) {
            ...
            if (card.value === 'A' && topCard.value === 'A' && card.suit === topCard.suit) isValid = true;
            else if (card.value === 'A' && topCard.value !== 'A') isValid = true;
...
function processCardLogic(card, currentPlayer) {
    ...
    if (card.value === '8') { advance = false; if(!isStart) currentPlayer.hasDrawn = false; }
        ...
        if (!isStart) {
            let target = playerOrder[(turnIndex + direction + playerOrder.length) % playerOrder.length];
            io.to(target).emit('notification', 'Άραξε 🍹');
        }
    }
...
function handleRoundEnd(winnerId, closedWithAce) {
    ...
    if (safe.length === 1 && playerOrder.length > 1) {
        io.emit('updateScoreboard', [...roundHistory, historyEntry]);
    ...
    roundHistory.push(historyEntry); io.emit('updateScoreboard', roundHistory);
